...
const HORIZONTAL_LINES_CLASSES = "-mx-8 stroke-2 text-stone-300 transition-colors group-hover:text-stone-400"

export async function SkateboardProduct ({ id }: Props) {
  const clienet = createClient()
  const product = await clienet.getByID<Content.SkateboardDocument>(id)
  ...
  return (
    <div className='group relative mx-auto w-full max-w-72 px-8 pt-4'>
      ...
      <HorizontalLine className={HORIZONTAL_LINES_CLASSES} />

      <div className='flex items-center justify-between ~text-sm/2xl'>
        <span>{price}</span>
        <span className="inline-flex items-center gap-1">
          <FaStar className='text-yellow-400' /> 30
        </span>
      </div>
      <div className="-mb-1 overflow-hidden py-4">
        <Scribble className='absolute inset-0 w-full h-full' color={dominantColor} />
        <PrismicNextImage alt='' field={product.data.image} width={150} className='mx-auto w-[58%] origin-top transition-transform duration-500 ease-in-out group-hover:scale-150' />
      </div>
